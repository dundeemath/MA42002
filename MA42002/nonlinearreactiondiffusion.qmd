---
title: "Travelling waves in linear reaction diffusion equations"
format: html
---
...
The numerical results motviate the following questions:
...
$$
Thus the wavespeed must be positive and the wave therefore travels in the positive direction along the $x$ axis.
...
Suppose that $v=0$. An explicit expresison for the trajectory is found by multiplying @eq- by $P$ and integrating over $z$. Hence
$$
\frac{P^2}{2}+ \int_0^W f(u)du=0.
$$
If this trajectory reached $W=1$ then
$$
\frac{P^2}{2}+ \int_0^1 f(u)du=0.
$$
...
Moreover, on this trajectory interects the W axis as $P<0$ implies that $W$ is decreasing.
Thus a trajectory with $v=0$ cannot connect $(1,0)$ and $(0,0)$.

Now suppose that $v$ is large.
