...
Steady states of @eq-fisher_unif are $u_1 =1$ and $u_2 =0$. To analyse the stability we consider 
$$
...
We can rewrite @eq-tw_eq as a system of two firest order ODEs
...
* Consider first $v=0$.
    ...
    * If  $W \in (a,1)$
    ...
    Thus the trajectory leaves $(1,0)$ with $ \dfrac{dP}{dW} >0$ along the unstable manifold $\textit{M}_u^{(1,0)}$ and intersects the line $\{ W=a\}$ at the point $(a, P_1)$. 
    ...
    \end{eqnarray*}
    Since $W(-\infty) =1$ we are moving along the unstable manifold $M_u^{(1,0)}$ and  $\dfrac{dW(z^\ast)}{dz}=P(z^\ast+ 0) = P_1$.
