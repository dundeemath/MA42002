...
\end{aligned}
$$ {#eq-n_1} Assuming that $N$ is differentiable, dividing @eq-n_1 by $\Delta t$ and taking the limit as $\Delta t \to 0$ $$
 \frac{dN}{dt} = K N 
$$ {#eq-n_2}
...
$$ {#eq-sol_c} where $\beta=c_0 +\alpha N_0$. Using @eq-sol_c in @eq-c_1 we obtain the *logistic growth* equation $$
\frac{dN}{dt} = \kappa ( \beta- \alpha N)  N, \qquad  N(0)= N_0\quad  
$$ {#eq-n_4} Here we have $K=K(N) = \kappa (\beta - \alpha N)$.

The last equation can be rewritten as $$  
\frac{dN}{dt} = \rho  N \,  (1 - \frac N B)  \qquad \quad N(0)= N_0, 
$$ {#eq-n_5} where $\rho = \kappa \beta$ is the **intrinsic growth rate** and $B = \frac \beta \alpha$ is the **carrying capacity**. The solution of @eq-n_5 is given by $$
...
Consider a model of infectious disease in which a population is slit into three compartments:

-   susceptible
-   infected
-   recovered

Suppse that interaction between susceptible and infected results in infection of susceptible. Suppose also that infected people only remain infectious for a limited time.
...
Consider a pair of interacting biochemical species, A and B. Suppose that both A and B are produced at a constant rate and that A undergoes linear degradation. Suppose also that A and B interact $$
2A+B \rightarrow A.
$$
