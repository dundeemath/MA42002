...
##  Notation
Throughout this course, we will study {\em partial differential equation models} of biological systems and as such we will consider $x \in \mathbb R^n$, $t \in [0, \infty)$ and functions $c: \mathbb R^n \times [0, \infty) \to \mathbb R$, where $n=1,2,3$. For example:
...
In this introductory first section, we shall neglect spatial movement and consider examples of growth/death and chemical reactions. Spatial movement will be considered in the next section and subsequently throughout the course. 
...
We can determine the nutrient concentration by solving equation @eq-C_1. We can integrate @eq-C_1 with respect to time and obtain
$$
\begin{equation}
...
Using @eq-sol_c in @eq-C_1 we obtain an equation just for $N$
$$
\begin{eqnarray}
...
\end{eqnarray}
$$ {#eq-N_5}
where $\rho = \kappa \beta$ is the {\em intrinsic growth rate} and $B = \frac \beta \alpha$ is the {\em carrying capacity}.
...
###  Chemical reactions
\textbf{Basic enzyme reactions} 

Most biochemical reactions involve proteins called {\em enzymes} which act as catalysts of chemical reactions. Enzymes react selectively on specific compounds called {\em substrates}.

A substrate $S$ reacts, in a {\em reversible reaction}, with an enzyme $E$ to form a {\em complex} $SE$ or $C$, which in turn is converted into a product $P$. This may be described as follows:
...
%$\longrightleftharpoons$  - reaction is reversible
...
* Taxis - directed movement in response to an external chemical or physical signal. 
    ...
    * haptotaxis - movement directed by a gradient in density, adhesion
    ...
    {\bf J} = \chi(a) c \nabla a,  
    $$
    where $\chi(a)$ is a `model-specific' function of $a$ defining the sensitivity to the signal, 
...
\textbf{Remark} We can also have different types of boundary conditions at different parts of the boundary of the considered domain. 
