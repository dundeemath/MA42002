...
In the mechano-chemical theory, cells interact with their surroundings and by exerting forces perturb their local environment. The combination of cell migration/proliferation and cell-generated forces is sufficient in certain circumstances to create a spatially heterogeneous distribution of cell densities i.e. the pattern is generated *simultaneously* with the cell migration/proliferation. This alternative pattern formation theory was proposed by Murray and Oster @murray1984generation.
...
In @fig-schnack you can see numerical solutions of the Schnackenberg model. The initial conditions are chosen to be close the spatially homogeneous steady state. AS time evolves we can see the emergence of pattern formation.

```{python}
#| echo: false
#| code-fold: true
#| label: fig-schnack
#| fig-cap: "Numerical solution of the Schnackenberg model."
...
v=np.zeros((N_t,N_x),dtype=float)

# Impose initial condition
# t=0 is the first row of the solution matrices

u[0,:]=(a+b)*np.ones_like(x)+0.01*np.random.uniform(low=-1.0, high=1.0, size=(N_x,))
v[0,:]=b*(1/(a+b)**2)*np.ones_like(x)+0.01*np.random.uniform(low=-1.0, high=1.0, size=(N_x,))

A_u=DefineA(Delta_t/(Delta_x**2))
A_v=DefineA(d*Delta_t/(Delta_x**2))

# Compute the inverse of A
A_inv_u=np.linalg.inv(A_u)
A_inv_v=np.linalg.inv(A_v)
...
for j in range(0,N_t-1):
    ...
    reaction_u, reaction_v=ComputeReactionRates(u[j,:],v[j,:])

    # Compute rhs of iterative scheme


    rhs_u=u[j,:]+Delta_t*reaction_u
    rhs_v=v[j,:]+Delta_t*reaction_v

    # MAtrix multiplication

    u[j+1,:]=(A_inv_u@(rhs_u.T)).T

    v[j+1,:]=(A_inv_v@(rhs_v.T)).T
...
# Full-width subplot at the top
ax2 = fig.add_subplot(gs[0:2, 2:4],projection='3d')       
# Larger subplot on the bottom-left
...
surf = ax1.plot_surface(x_mesh, t_mesh,u,linewidth=0,cmap=cm.coolwarm, antialiased=False)
...
surf = ax2.plot_surface(x_mesh, t_mesh,v,linewidth=0,cmap=cm.coolwarm, antialiased=False)
...
ax3.plot(x,u[-1,:],x,v[-1,:])

ax3.set_xlim([0,L])
ax3.set_xlabel('$x$')
ax3.set_title('')
ax3.legend(['$u$','$v$'])
ax2.set_title('Solution at time $T$')
...
with no-flux boundary condition on $x=0$ and $x=1$.
The solution is
$$
...
#### Instability of spatially heterogeneous perturbations

we can look for the spatial component of the solution of @eq-pp_4 as follows: 
$$
...
-   $d < d_c$, ; $Re \lambda (k^2) \< 0, \forall k\^2 \$, and there is no pattern;
