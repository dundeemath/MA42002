...
format: html
---
\subsection{\Large Mathematical modelling of infection diseases (SIR)}{$\phantom{aaaaa}$}
 ...
 Assumptions
 \begin{itemize}
\item   Total population is constant: the duration of the epidemic is short compared to the lifetime of its hosts, so we can neglect birth and disease-unrelated death

\item Consider a disease which, after recovery, confers immunity (and/or death if lethal)  

\item  Simple diffusion for spatial distribution of population 
  \end{itemize}
 ...
 \begin{itemize}
\item  $S$ -- susceptibles - such that can be infected
 \item  $I$ -- invectives  - such that have the disease and can transmit it to susceptibles
\item  $R$ -- recovered (removed)  - such that have had the disease and are immune or isolate until recovered or death (if the disease is lethal) 
  \end{itemize}
...
Model assumptions 
 \begin{itemize}
\item The gain in the invectives class is at the rate proportional to the number of invectives $I$ and susceptibles $S$, i.e. $r\;  I\; S$, \; $r>0$.  

\item  The susceptibles are lost at the same rate, i.r. $r\; I\; S$
\item The rate of removal of invectives to the recovered class $R$ is proportional to the number of invectives, i.e. $a\; I$, \; $a>0$. 

$\dfrac 1 a $ measures the time spent in the infectious state.
 
 \item The incubation period is short enough to be negligible: susceptibles are directly infected after coming into contact with the disease (with invectives). 
 
\end{itemize}

Then a simple SIR model reads  ( in a long thin domain or in $3$-dim domain with solutions in a form of planar fronts) 
\begin{equation}\label{SIR1}
\begin{aligned}
...
where
\begin{itemize} 
\item  $a>0$ -- removal or death rate 
\item $r>0$ --  transmission or infection rate 
\item  $D_S>0$, \; $D_I>0$, \; $D_R>0$ -- diffusion coefficients 
\end{itemize}
...
\end{aligned}
\end{equation}
where $\bar S_0$ is a representative population density and $\mu = \dfrac a { r \bar S_0}$.
...
\begin{equation}\label{SIR_TW_BC}
\begin{aligned}
...
\end{equation}
 ...
 $$
Considering boundary conditions \eqref{SIR_TW_BC} we obtain 
two steady states 
$$
...
Hence we would like to have a heteroclinic connection between $(\sigma , 0)$ and $(1,0)$. 
 ...
 \eqref{SIR_TW} and \eqref{SIR_TW_BC} is 
 \begin{equation}\label{cond_v_SIR}
...
$$
Thus 
\begin{equation}\label{i_TW_infty}
...
\begin{equation}\label{SIR_TW_BC_2}
\begin{aligned}
s(z) \to 1 \qquad  z\to + \infty, \quad \qquad  i(z) \to 0 \qquad  z\to + \infty\; ,\\
s(z) \to \sigma \qquad  z\to - \infty, \quad \qquad  i(z) \to 0 \qquad  z\to - \infty\; ,\\
s^\prime(z) \to 0\qquad  z\to \pm \infty, \quad \qquad i^\prime(z) \to 0 \qquad z \to \pm \infty \; ,
...
\end{equation}
 ...
 is =0 , \quad i ( s- \mu) = 0  \quad \Longrightarrow \quad  i=0, \quad s = \text{const} \; .
 $$
Considering boundary conditions \eqref{SIR_TW_BC_2} we obtain 
two steady states 
$$
(s_0, i_0) = ( 1, 0), \qquad (s_0, i_0) = (\sigma, 0) \; .
$$

Linearising equations \eqref{SIR_TW_2} about the steady state $(1,0)$ and requiring that $i$ is nonnegative we obtain, as for \eqref{SIR_TW}, 
 the necessary conditions  for existence of  travelling wave solutions satisfying 
  \eqref{SIR_TW_2} and  \eqref{SIR_TW_BC_2} :
  $$
 v \geq 2 \sqrt{ 1- \mu} \, \quad \text{ and } \quad 0 \leq \mu < 1\; .  
 $$
...
Using first equation in \eqref{SIR_TW_2} in the second implies 
...
\end{equation}
Consider now ageing the first equation in \eqref{SIR_TW_2}
$$
i= v \frac {s^\prime} s, \quad \quad s \neq 0 \; 
$$
and obtain from \eqref{sigma_2}
\begin{equation*}
 i^{\prime } + v i + v s  - v  \mu \int   \frac {s^\prime} s\, dz = K= const\; .
\end{equation*}
or
\begin{equation}\label{sigma_3}
 i^{\prime } + v i + v s  - v  \mu \ln(s) = K\; .
\end{equation}
Using now in \eqref{sigma_3} boundary conditions as $z \to + \infty$,  from    \eqref{SIR_TW_BC_2},   we can determine constant $K$:
\begin{equation*}
 v    = K\; .
\end{equation*}
Thus we have 
\begin{equation}\label{sigma_4}
 i^{\prime } + v i + v( s  - \mu \ln(s) -1)= 0\; .
\end{equation}
Using now in \eqref{sigma_4} boundary conditions as $z \to - \infty$, see   \eqref{SIR_TW_BC_2},  gives 
\begin{equation*}
 v( \sigma - \mu \ln(\sigma) -1)= 0\; .
\end{equation*}
and 
\begin{equation}\label{sigma_5}
...
\item 
 ...
 \item
 or if  infection rate $r$ is too small - the disease is too weak and no wave of infection is possible. 
 \end{itemize} 
 
 \subsubsection{\bf Generalisation of simple SIR model} 
...
We generalised the simple model by considering growth of susceptibles population in a logistic manner 
 \begin{equation}\label{SIR_growth}
\begin{aligned}
...
We can non-dimensionalize \eqref{SIR_growth} as before and obtain 
 \begin{equation}\label{SIR_growth_nd}
\begin{aligned}
...
\end{aligned}
\end{equation}
 where $b= \dfrac{B}{r S_0}$.
 
Spatially homogeneous  steady states of  \eqref{SIR_growth_nd} are 
 ...
 \begin{equation}\label{SIR_TW_growth}
\begin{aligned}
...
\end{equation}
 and by introducing new variable $w= i^\prime$ obtain 
  \begin{equation}\label{SIR_TW_growth_2}
\begin{aligned}
...
\end{aligned}
\end{equation}
The system  \eqref{SIR_TW_growth_2} has two stationary  solutions 
 $(s^\ast_1, i^\ast_1, w^\ast_1) = (1,0,0)$ and  $(s^\ast_2, i^\ast_2, w^\ast_2) = (\mu, b(1-\mu),0)$.
Considering linearisation of  \eqref{SIR_TW_growth_2} and computing eigenvalues of Jabocian matrix 
$$
...
$$ 
 ...
 $(s^\ast_1, i^\ast_1, w^\ast_1) = (1,0,0)$  is a saddle point and 
 $(s^\ast_2, i^\ast_2, w^\ast_2) = (\mu, b(1-\mu),0)$ is a stable node for $\mu < \mu^\ast$ and a stable spiral (focus) for $\mu >\mu^\ast$, with some threshold value $\mu^\ast$. 
...
\subsection{\bf Pattern formations in chemotaxis model}
 We shall consider a minimal model for the aggregation phase.  
Starved slime hold amoebae sect  a chemical cyclin AMP (cAMP) that attract other cells. 
Cells also secrete phosphodiesterase, an enzyme that degrades cAMP. \\


\textbf{Model assumptions}
\begin{itemize}
\item Individual cells undergo combination of random motion and chemotaxis towards cAMP
\item Cells neither die nor divide during aggregation 
\item Attractant cAMP is produced at a constant rate by each cell 
\item The rate of degradation of cAMP depends linearly on its concentration 
\item cAMP diffuses passively 
\end{itemize} 
...
Thus we obtain chemotaxis system  in a domain $\Omega \subset \mathbb R^n$ with sufficiently smooth boundary 
\begin{equation}
\begin{aligned}
...
\end{aligned}
\end{equation}
where $D_a>0$, $D_c>0$ diffusion coefficients,  $\beta>0$ - rate of cAMP secretion by cells per unit density, $d>0$ - rate of degradation of cAMP, $\textbf{n}$ - outer normal vector to $\partial \Omega$. 
%\frame{
%\frametitle{Positively invariant domains} 
%{\color{blau} \bf Definition} A closed subset $M\subset \mathbb R^n$ is called a {\color{gruen} positively invariant region} for solutions of  (a system of ODE)
%\begin{align}\label{ode_nonlin_2}
%\begin{aligned}
%\frac{du}{dt} &= F(t,u(t)), \qquad u\in  \mathbb R^n, \\
%u(0)&= u_0 
%\end{aligned}
%\end{align}
%if any solution $u(t)$, having initial values $u_0$ in $M$, satisfies $u(t) \in M$ for all $t\geq 0$ .
%%( if $u_0\in M$ implies that $u(t, u_0) \in M$ for all $t\geq 0$ ).
%
%{\color{blau} \bf Theorem }
%Let $\Phi \in C^1(U, \mathbb R)$ such that $\nabla \Phi(u) \neq 0$ for all $u\in \Phi^{-1}(0)$, that is assume that $0$ is a regular value of $\Phi$.  \\
%Then the region  $M:=\{u \in U: \Phi(u) \leq 0 \} $ is positively invariant for solutions of \eqref{ode_nonlin_2} if and only if  
%$$
%(\nabla \Phi(u), F(u)) \leq 0 \quad \quad \forall u \in \partial M = \Phi^{-1}(0).
%$$
%
%Let $\Phi_i \in C^1(U, \mathbb R)$ and $0$ is a regular value of $\Phi_i$,  with  $i=1, \dots k$. Consider $M:=\cap_{i=1}^k\{u \in U: \Phi_i(u) \leq 0 \}$.
%If we have 
%$$
