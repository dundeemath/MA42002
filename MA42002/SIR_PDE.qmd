...
By imposing the constraint that (1,0) cannot be a spiral, a necessary condition for the existence of travelling wave solutions satisfying @eq-sir_tw and @eq-sir_tw_bc is
...
### The leading edge of the epidemic front 
...
$$ {#eq-i_tw_infty} We can also show that the travelling wave solution $s(z)$ cannot have a local maximum, since for $s^\prime(z) = 0$ first equation in @eq-sir_tw implies 
$$
...
$$ 
Then using @eq-i_tw_infty we can conclude that
...
Using substituting the first equation in @eq-sir_tw_2 in the second implies 
$$
...
Consider now rearranging the first equation in @eq-sir_tw_2 
$$
